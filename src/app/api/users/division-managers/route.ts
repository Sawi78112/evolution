...
export async function GET(request: NextRequest) {
  try {
    ...
    // First, get the Divisional Manager role ID
    const { data: roleData, error: roleError } = await supabase
      .from('roles')
      .select('role_id')
      .eq('role_name', 'Divisional Manager')
      .single();

    if (roleError || !roleData) {
      console.error('Failed to fetch Divisional Manager role:', roleError);
      return NextResponse.json(
        { 
          error: 'Divisional Manager role not found',
          details: roleError?.message || 'No Divisional Manager role in database'
        },
        { status: 404 }
      );
    }
