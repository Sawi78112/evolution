...
import { AddCaseTabNavigation, type AddCaseTab } from './AddCaseTabNavigation';
import { TabPlaceholder } from './TabPlaceholder';
import { CaseDetailTab } from './CaseDetailTab';
import { AddCaseModalProps } from './types';
import { useNotification } from '@/components/ui/notification';

export function AddCaseModal({ isOpen, onClose, onSuccess }: AddCaseModalProps) {
  const [mounted, setMounted] = useState(false);
  const [activeTab, setActiveTab] = useState<AddCaseTab>('case-detail');
  ...
  const handleSubmit = async (formData: any) => {
    ...
    try {
      console.log('Creating case with data:', JSON.stringify(formData, null, 2));
      ...
      notification.error(
        ...
        error instanceof Error ? error.message : 'An unexpected error occurred while creating the case.'
      );
    } finally {
    ...
    >
      <div className="no-scrollbar relative w-full max-w-[1300px] max-h-[95vh] overflow-y-auto rounded-3xl bg-white p-6 dark:bg-gray-900">
        ...
        <div className="mb-4">
          ...
          <AddCaseTabNavigation activeTab={activeTab} onTabChange={setActiveTab} />
        </div>
        ...
        <div className="mb-4">
          ...
          {activeTab === 'case-detail' && (
            <CaseDetailTab onSubmit={handleSubmit} onClose={onClose} isSubmitting={isSubmitting} />
          )}
