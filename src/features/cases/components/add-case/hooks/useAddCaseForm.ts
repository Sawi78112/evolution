...
export function useAddCaseForm() {
  ...
  // Auto-generate coordinates when location is complete
  useEffect(() => {
  ...
  const generateCoordinates = async () => {
    ...
    try {
      // Try to use a geocoding service (you can replace this with your preferred service)
      const locationString = [formData.address, formData.city, formData.state, formData.country]
        .filter(Boolean)
        .join(', ');
